...
if(isset($_GET['question'])){
	...
	$answers = "";
	$q = "";
	$sql = mysql_query("SELECT id FROM quiz_questions");
	...
	$singleSQL = mysql_query("SELECT * FROM quiz_questions WHERE id='$question' LIMIT 1");
		while($row = mysql_fetch_array($singleSQL)){
			...
			$sql2 = mysql_query("SELECT * FROM answers_questions WHERE question_id='$question' ORDER BY rand()");
			while($row2 = mysql_fetch_array($sql2)){
				$answer = $row2['answer'];
				$correct = $row2['correct'];
				$answers .= '<label style="cursor:pointer;"><input type="radio" name="radio" value="'.$correct.'">'.$answer.'</label> 
